...
def command_network(name, a, b):
    return 'qsub -v NAME=' + name + ',START=' + str(a) + ',END=' + str(b) + to_qsub + 'qsub_network.sh'
...
def run_network():
    ...
    for i in range(0, no_slices):
        ...
        print(command_network(name, start, end))
        if sys.platform.startswith('linux'):
            os.system(command_network(name, start, end))

    print('remaining: ')
    ### if some remain...
    while last_sub < last:
        print(command_network(name, last_sub, last_sub+1))
        if sys.platform.startswith('linux'):
            os.system(command_network(name, last_sub, last_sub+1))
        last_sub +=1
